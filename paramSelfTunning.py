...
def SA(islandNumber, expectedProgression, currentGen, maxTempGen, definitions, progressions, parametersToProgress,
       current_fitness):
    temperature = mapT(currentGen, 0, maxTempGen, 1e4,
                       0)  # simulação de temperatura diminuindo (0-N para 0-1e4) desnec?
    for i, parameter in zip(range(len(parametersToProgress)), parametersToProgress):
        ...
        if (deltaE <= 0 or (deltaE < expectedProgression)):  # progresso piorou ou não mudou
            ...
            if (random.random() >= ap):
                ...
                for j in range(100):
                    if (type == 'float'):
                        A.append(random.uniform(lower_limit,
                                                upper_limit))  # gerar uma lista de valores que o parâmetro pode ter
                    else:
                        A.append(random.randint(lower_limit, upper_limit))
                A = list(A)
