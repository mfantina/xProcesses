...
def calculateTPTask(auxCrom, cromossome, i, numberOfHiddenTask):   # added "numberOfHiddenTask" [hiddenTasks]
    intersectionModelAndLog = 0
    modelSum = 0
    for j in range(2, len(cromossome[i]) - 3 - (2 * numberOfHiddenTask), 2):   # era "len(cromossome[i]) - 3" [hiddenTasks]
...
def calculateTP(cromossome, referenceCromossome, numberOfHiddenTask):   # added "numberOfHiddenTask" [hiddenTasks]
    TP = 0
    for i in range(0, len(cromossome) - 5 - (2 * numberOfHiddenTask), 2):   # era "len(cromossome) - 5" [hiddenTasks][hiddenTasks]
        TP = TP + calculateTPTask(referenceCromossome, cromossome, i, numberOfHiddenTask)
    return TP/((len(cromossome) - 5 - (2 * numberOfHiddenTask)) / 2)   # era "len(cromossome) - 5" [hiddenTasks][hiddenTasks]
        ...
        if topInputGatewayType == 0:
            if bottomInputGatewayType == 0:
                if inputIndexes.count(tokens[index1][0]) > 0:  # 1
                    ...
                    #if tokens[index1][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!!  por que aqui não aceita essa restrição?!
                    ...
                    if topInputIndexes.count(tokens[index1][0]) > 0:  # 2
                        if (numberOfANDTokenOutputs == 1 and activityFoundInxORorANDOutputToken == 1) or (numberOfANDTokenOutputs == 2):
                            for m in range(len(tokens[index1][1][side])):
                                if tokens[index1][1][side][m] != initPop.getTaskID(log[i][l], alphabet):
                                    tokens.append([tokens[index1][0], [tokens[index1][1][side][m]]])
                        #if 1 == 1:  #if tokens[index1][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!!
                        tokens.remove(tokens[index1])
                        availableToken = 1
                        properlyParsedTasks = properlyParsedTasks + 1
                        return (inputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, 1, parsedTasks, enabledTasks)
                ...
                if bottomInputGatewayType == 1:
                    if (((topInputIndexes.count(tokens[index1][0]) > 0) or topInputIndexes == [-1]) and ((bottomInputIndexes.count(tokens[index2][0]) > 0) or bottomInputIndexes == [-1])):  # 5
                        ...
                        if (topInputIndexes != [-1]) and (bottomInputIndexes != [-1]):
                            ...
                            if tokens[max(index1, index2)][1] != []:
                                if tokens[max(index1, index2)][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 1
                                    tokens.remove(tokens[max(index1, index2)])
                                    if index1 != index2:
                                        if tokens[min(index1, index2)][1] != []:
                                            if tokens[min(index1, index2)][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 2
                            ...
                            if (topInputIndexes != [-1]) and (bottomInputIndexes == [-1]):
                                ...
                                if tokens[index1][1] != []:
                                    if tokens[index1][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 3
                                        tokens.remove(tokens[index1])
                            else:
                                ...
                                waitingTokens = treatWaitingTokens(waitingTokens, tokens, bottomInputIndexes, index2)
                                if tokens[index1][1] != []:
                                    if tokens[index1][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 4
                                        tokens.remove(tokens[index1])
                    ...
                    if bottomInputGatewayType == 0:
                        if (((topInputIndexes.count(tokens[index1][0]) > 0) or topInputIndexes == [-1]) and ((bottomInputIndexes.count(tokens[index2][0]) > 0) or bottomInputIndexes == [-1])):  # 6
                            ...
                            if (topInputIndexes != [-1]) and (bottomInputIndexes != [-1]):
                                ...
                                if (len(topInputIndexes) > 0):
                                    if tokens[index1][1] != []:
                                        if tokens[index1][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 5
                                            tokens.remove(tokens[index1])
                                else:
                                    if tokens[max(index1, index2)][1] != []:
                                        if tokens[max(index1, index2)][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 6
                                            tokens.remove(tokens[max(index1, index2)])
                                            if index1 != index2:
                                                if tokens[min(index1, index2)][1] != []:
                                                    if tokens[min(index1, index2)][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 7
                                                        tokens.remove(tokens[min(index1, index2)])
                            if len(topInputIndexes) == 0:
                                availableToken = 1
                                properlyParsedTasks = properlyParsedTasks + 1
                    ...
                    if bottomInputGatewayType == 1:
                        if (((topInputIndexes.count(tokens[index1][0]) > 0) or topInputIndexes == [-1]) and ((bottomInputIndexes.count(tokens[index2][0]) > 0) or bottomInputIndexes == [-1])):  # 7
                            ...
                            if (topInputIndexes != [-1]) and (bottomInputIndexes != [-1]):
                                ...
                                if len(bottomInputIndexes) > 0:
                                    if tokens[index2][1] != []:
                                        if tokens[index2][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 8
                                            tokens.remove(tokens[index2])
                                else:
                                    if tokens[max(index1, index2)][1] != []:
                                        if tokens[max(index1, index2)][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 9
                                            tokens.remove(tokens[max(index1, index2)])
                                            if index1 != index2:
                                                if tokens[min(index1, index2)][1] != []:
                                                    if tokens[min(index1, index2)][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  # new!!! 10
                                                        tokens.remove(tokens[min(index1, index2)])
                            if len(bottomInputIndexes) == 0:
                                availableToken = 1
                                properlyParsedTasks = properlyParsedTasks + 1
                        ...
                        if bottomInputGatewayType == 0: # 8
                            if (((topInputIndexes.count(tokens[index1][0]) > 0) or topInputIndexes == [-1]) and ((bottomInputIndexes.count(tokens[index2][0]) > 0) or bottomInputIndexes == [-1])):
                                ...
                                if (topInputIndexes != [-1]) and (bottomInputIndexes != [-1]):
                                    if tokens[max(index1, index2)][1] != []:
                                        if tokens[max(index1, index2)][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 11
                                            tokens.remove(tokens[max(index1, index2)])
                                            if index1 != index2:
                                                if tokens[min(index1, index2)][1] != []:
                                                    if tokens[min(index1, index2)][1][0] == initPop.getTaskID(log[fixed_i][fixed_l], alphabet):  #new!!! 12
                                                        tokens.remove(tokens[min(index1, index2)])
                                availableToken = 1
                                properlyParsedTasks = properlyParsedTasks + 1
                                return (inputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, 1, parsedTasks, enabledTasks)
    ...
    while (len(tokens) > 0) and (count < (completenessAttemptFactor1 + pow(len(tokens), completenessAttemptFactor2))):
        ...
        if numberOfANDTokenOutputs == 0:
            if tokens[index1][1].count(initPop.getTaskID(log[i][l], alphabet)) > 0:
                (inputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, breakSignal, parsedTasks, enabledTasks) = processTokensToOutpus(parsedTasks, enabledTasks, numberOfANDTokenOutputs, centralInputGatewayType, topInputGatewayType, bottomInputGatewayType, inputIndexes, topInputIndexes, bottomInputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, index2, 0, 0, 0, 0, log, alphabet, i, l)
                if breakSignal == 1:
                    break
        else:
            if numberOfANDTokenOutputs == 1:
                if tokens[index1][1][0].count(initPop.getTaskID(log[i][l], alphabet)) > 0:
                    (inputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, breakSignal, parsedTasks, enabledTasks) = processTokensToOutpus(parsedTasks, enabledTasks, numberOfANDTokenOutputs, centralInputGatewayType, topInputGatewayType, bottomInputGatewayType, inputIndexes, topInputIndexes, bottomInputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, index2, i, l, 0, 1, log, alphabet, i, l)
                    if breakSignal == 1:
                        break
                else:
                    if tokens[index1][1].count(initPop.getTaskID(log[i][l], alphabet)) > 0:
                        (inputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, breakSignal, parsedTasks, enabledTasks) = processTokensToOutpus(parsedTasks, enabledTasks, numberOfANDTokenOutputs, centralInputGatewayType, topInputGatewayType, bottomInputGatewayType, inputIndexes, topInputIndexes, bottomInputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, index2, i, l, 0, 0, log, alphabet, i, l)
                        if breakSignal == 1:
                            break
            else:
                if numberOfANDTokenOutputs == 2:
                    if tokens[index1][1][0].count(initPop.getTaskID(log[i][l], alphabet)) > 0:
                        ...
                        if tokens[index1][1][1].count(initPop.getTaskID(log[i][l], alphabet)) > 0:
                            (inputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, breakSignal, parsedTasks, enabledTasks) = processTokensToOutpus(parsedTasks, enabledTasks, numberOfANDTokenOutputs, centralInputGatewayType, topInputGatewayType, bottomInputGatewayType, inputIndexes, topInputIndexes, bottomInputIndexes, tokens, waitingTokens, properlyParsedTasks, availableToken, index1, index2, i, l, 1, 0, log, alphabet, i, l)
                            if breakSignal == 1:
                                break
    if availableToken == 0:
...
def calculateSimplicity(cromossome, alphabet):
    ...
    for i in range(len(alphabet)):  
        ...
        for j in range(len(alphabet)):              # Era "len(alphabet) - 1"  [hiddenTasks]
...
def adaptCromossome(cromossome, alphabet):
    for i in range(int((len(cromossome) - 3) / 2)):                                                                     # era "len(alphabet)" [hiddenTasks]
        numberOfL1 = 0
        numberOfR1 = 0
        for j in range(int((len(cromossome) - 3) / 2)):                                                                 # era "len(alphabet)" [hiddenTasks]
            if (cromossome[i * 2][j * 2] == 1) or (cromossome[(i * 2) + 1][j * 2] == 1):
                numberOfL1 = numberOfL1 + 1
                if numberOfL1 == 2:                                                                                     # para [hiddenTasks] eu passei pra 2000, mas voltei para 2?????
                    break
        for j in range(int((len(cromossome) - 3) / 2)):                                                                 # era "len(alphabet)" [hiddenTasks]
            if (cromossome[i * 2][(j * 2) + 1] == 1) or (cromossome[(i * 2) + 1][(j * 2) + 1] == 1):
                numberOfR1 = numberOfR1 + 1
                if numberOfR1 == 2:                                                                                     # para [hiddenTasks] eu passei pra 2000, mas voltei para 2 ?????
                    break
    ...
    for i in range(int((len(cromossome) - 3) / 2)):                                                                     # era "len(alphabet)" [hiddenTasks]
        ...
        for j in range(int((len(cromossome) - 3) / 2)):                                                                 # era "len(alphabet)" [hiddenTasks]
            if (cromossome[j * 2][i * 2] == 1) or (cromossome[j * 2][(i * 2) + 1] == 1):
                ...
                if numberOfT1 == 2:                                                                                     # se em cima eu havia passado para [hiddenTasks] para 2000, por que aqui não?
                    break
        for j in range(int((len(cromossome) - 3) / 2)):                                                                 # era "len(alphabet)" [hiddenTasks]
            if (cromossome[(j * 2) + 1][i * 2] == 1) or (cromossome[(j * 2) + 1][(i * 2) + 1] == 1):
                numberOfB1 = numberOfB1 + 1
                if numberOfB1 == 2:                                                                                     # se em cima eu havia passado para [hiddenTasks] para 2000, por que aqui não?
                    break
...
def evaluateIndividual(cromossome, referenceCromossome, TPweight, precisenessWeight, simplicityWeight, completenessWeight, i, completenessAttemptFactor1, completenessAttemptFactor2, alphabet, log, numberOfHiddenTask):
    TP = calculateTP(cromossome, referenceCromossome, numberOfHiddenTask)                                               # added "numberOfHiddenTask" [hiddenTasks]
    completeness = 0
    precisenessAKM = 0
    precisenessBVB = 0
    simplicity = 0
...
def evaluationPopulation(population, referenceCromossome, TPweight, precisenessWeight, simplicityWeight, completenessWeight, completenessAttemptFactor1, completenessAttemptFactor2, selectionOp, alphabet, log, numberOfHiddenTask):
    evaluationSum = 0
    evaluationValues = []
    for i in range(len(population)):
        evaluationValues.append(evaluateIndividual(population[i], referenceCromossome, TPweight, precisenessWeight, simplicityWeight, completenessWeight, i, completenessAttemptFactor1, completenessAttemptFactor2, alphabet, log, numberOfHiddenTask))
