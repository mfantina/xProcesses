...
def BVBCrossover(crossoverTasksNumPerc, cromossome1, cromossome2, i, evaluatedPopulation, alphabet):
    ...
    if crossoverTasksNumPerc == -1:
        ...
        tasksNum = int(crossoverTasksNumPerc * ((len(cromossome1) - 3) / 2))    #Era "len(alphabet)"    [hiddenTasks]
    for i in range(tasksNum):
        if (len(incorreclyFiredTasks)) > 0:
            chosenTask = int(ran.choice(incorreclyFiredTasks))
        else:
            chosenTask = int(ran.random() * ((len(cromossome1) - 3) / 2))    #Era "len(alphabet)"    [hiddenTasks]
...
def uniformCrossoverPerProcess(cromossome1, cromossome2, alphabet):
    offspring1 = copy.deepcopy(cromossome1)
    offspring2 = copy.deepcopy(cromossome2)
    for i in range(0, ((len(cromossome1) - 3) / 2)):    #Era "len(alphabet)"    [hiddenTasks]
        if ran.random() < 0.5:
...
def twoPointCrossoverPerProcess(cromossome1, cromossome2, alphabet):
    offspring1 = copy.deepcopy(cromossome1)
    offspring2 = copy.deepcopy(cromossome2)
    cutpoint1 = int(ran.random() * ((len(cromossome1) - 3) / 2))    #Era "len(alphabet)"    [hiddenTasks]
    cutpoint2 = int(ran.random() * (((len(cromossome1) - 3) / 2) - cutpoint1))    #Era "len(alphabet)"    [hiddenTasks]
    ...
    for i in range(0, cutpoint1):
        ...
        for j in range(len(cromossome1)):
            ...
            offspring2[j][(i * 2)] = cromossome1[j][(i * 2)]
            offspring2[j][(i * 2) + 1] = cromossome1[j][(i * 2) + 1]
    for i in range(cutpoint2 + 1, ((len(cromossome1) - 3) / 2)):    #Era "len(alphabet)"    [hiddenTasks]
        offspring1[(i * 2)] = cromossome2[(i * 2)]
        offspring1[(i * 2) + 1] = cromossome2[(i * 2) + 1]
        offspring2[(i * 2)] = cromossome1[(i * 2)]
        offspring2[(i * 2) + 1] = cromossome1[(i * 2) + 1]
...
def singlePointCrossover(cromossome1, cromossome2, alphabet):
    offspring1 = copy.deepcopy(cromossome1)
    offspring2 = copy.deepcopy(cromossome2)
    cutpoint = int(ran.random() * ((len(cromossome1) - 3) / 2))    #Era "len(alphabet)"    [hiddenTasks]
...
def mutationBVB(mutationTasksNumPerc, cromossome, alphabet):
    if mutationTasksNumPerc == -1:
        ...
        tasksNum = int(mutationTasksNumPerc * ((len(cromossome) - 3) / 2))    #Era "len(alphabet)"    [hiddenTasks]
        if tasksNum < 1:
            tasksNum = 1
    for i in range(tasksNum):
        chosenTask = int(ran.random() * ((len(cromossome) - 3) / 2))    #Era "len(alphabet)"    [hiddenTasks]
        chosenType = ran.random()
        if chosenType < 1/3:
            position1 = ran.randrange(0, 1 + 1)
            position2 = ran.randrange(0, 1 + 1)
            newInput = int(ran.random() * ((len(cromossome) - 3) / 2))    #Era "len(alphabet)"    [hiddenTasks]
            ...
            if chosenType > 2/3:
                position1 = ran.randrange(0, 1 + 1)
                position2 = ran.randrange(0, 1 + 1)
                newOutput = int(ran.random() * ((len(cromossome) - 3) / 2))    #Era "len(alphabet)"    [hiddenTasks]
